...
{   
    ...
    template<class T, typename IDX = u16> class IArray
    {
    protected:
        IArray(T* const buffer, const IDX capacity)
            : bufferPtr { buffer }
            , COUNT_MAX  { capacity }
            , count     { 0 }
        {}
    ...
    public:
        [[nodiscard]] T&       operator[](const IDX i)        { CHECK_INDEX(i, count);       return bufferPtr[i]; } 
        [[nodiscard]] const T& operator[](const IDX i) const  { CHECK_INDEX(i, count);       return bufferPtr[i]; }
        ...
        [[nodiscard]] IDX Count() const                       { return count; }
        ...
        template<class... Args>
        T& Insert(const IDX insertIdx, Args&&... args)
        {
            if (count == 0)
    ...
    public:
        const IDX COUNT_MAX;
        using DATA_TYPE  = T;
        using COUNT_TYPE = IDX;
    };


    template<class T, std::size_t CAPACITY_T, typename IDX = u16>
    class Array final : public IArray<T, IDX>
    {
    public:
        static constexpr auto SIZE = CAPACITY_T * sizeof(T);

        Array() : IArray<T, IDX> { reinterpret_cast<T* const>(buffer), CAPACITY_T } { ; }
        ...
        ANY_SIZE void operator=(const A& other)
        {
            ...
            for (IDX i = 0; i < this->count; ++i) ctor(i, other[i]);
        }
        ...
        {
            this->count = (CAPACITY_T >= other.Count()) ? other.Count() : CAPACITY_T;
            for (IDX i = 0; i < this->count; ++i) ctor(i, std::move(other[i]));
        }
