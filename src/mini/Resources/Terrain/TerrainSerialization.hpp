...
void WriteOrRead(QUADRANT_T (&quadrants)[QUADRANT_COUNT][QUADRANT_COUNT])
{
    ...
    for(idx_t x = 0; x < QUADRANT_COUNT; ++x) {
        ...
        if constexpr (DoSave)  file.write((const char*)quadrant.verts, sizeof(utils::Common_Vertex) * quadrant.VERT_COUNT_TOTAL);
        else                   file.read((char*)quadrant.verts, sizeof(utils::Common_Vertex) * quadrant.VERT_COUNT_TOTAL);
    }}
}
