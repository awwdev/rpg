...
namespace mini::vk
{
    struct VkRenderer //: rendergraph::IRenderer
    {
        ...
        void UpdateVkResources(const app::Scene& scene, const double dt, rendergraph::RenderGraph& renderGraph)
        {
            ...
            rendergraph::UniformData_Text arr [3] = 
            {
                //color index points into color array that is const in shader
                { { 32*1, 32*1, 0}, 0, 0 },
                { { 32*2, 32*2, 0}, 1, 1 },
                { { 32*3, 32*3, 0}, 2, 2 },
            };
            resources.text_ubo.buffer.Store(arr, sizeof(rendergraph::UniformData_Text) * 3);
        ...
        inline void RecordCommands(const uint32_t cmdBufferIdx, const double dt, const app::Scene& scene)
        {
            ...
            vkCmdDraw               (cmdBuffer, 6*3, 1, 0, 0); //TODO: get count somewhere
